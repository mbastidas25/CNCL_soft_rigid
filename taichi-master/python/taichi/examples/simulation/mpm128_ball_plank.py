...
def reset():
    ...
    # Ball
    for i in range(n_ball_particles):
        r = ti.sqrt(ti.random()) * 0.1  # Radius
        theta = ti.random() * 2 * 3.141529  # Angle
        x[i] = ti.Vector([0.5 + r * ti.cos(theta), 0.7 + r * ti.sin(theta)])  # Even distribution within a circle
        material[i] = 1  # jelly
        v[i] = [0, 0]
    ...
    # Rectangle (Plank)
    angle = 3.141529 / 6  # 30 degrees tilt
    rotation_matrix = ti.Matrix([[ti.cos(angle), -ti.sin(angle)], [ti.sin(angle), ti.cos(angle)]])
    for i in range(n_rectangle_particles):
        idx = i + n_ball_particles  # Ensure we don't overwrite ball particles
        local_pos = ti.Vector([
            ti.random() * 0.1 - 0.05,  # Adjust range for shorter plank
            ti.random() * 0.02 - 0.01
        ])
        rotated_pos = rotation_matrix @ local_pos + ti.Vector([0.5, 0.2])
        x[idx] = rotated_pos
        material[idx] = 3  # Assign the new material type for the rigid plank
        v[idx] = [0, 0]
